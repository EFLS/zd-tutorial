...
Use =C-c d /= (for =zetteldeft-search-tag=) to interactively select a tag and search for it.
Or generate an overview of all tags in a new buffer with =C-c d T= (for =zetteldeft-tag-buffer=).
